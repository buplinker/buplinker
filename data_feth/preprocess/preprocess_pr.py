...
from data_fetch.preprocess.template_extractor import TemplateExtractor
...
def get_cache_info(logger=None):
    """キャッシュファイルの情報を取得"""
    cache_files = []
...
def resolve_description_links(repository_id: int, description: str, description_html: str, processed_refs: set = set(), logger=None) -> str:
    ...
    processed_html = description_html
    ...
    while True:
        # hrefパターンを探す
        match = re.search(data_url_pattern, processed_html)
        if not match:
            break
        ...
        if url in processed_urls:
            # 処理済みのURLを削除して次を探す
            processed_html = processed_html.replace(match.group(0), '', 1)
            continue
...
def _process_issue_reference(repository_id: int, issue_number: int, processed_refs: set, logger=None) -> str:
    ...
    try:
        ...
        if issue:
            ...
            # テンプレート削除を適用
            processed_issue_body = extract_template_content_for_single_text(
            ...
            processed_issue_body = processed_issue_body or ""
...
def _process_pull_request_reference(repository_id: int, pull_number: int, processed_refs: set, logger=None) -> str:
    ...
    try:
        ...
        if pull_request:
            ...
            # テンプレート削除を適用
            processed_pr_body = extract_template_content_for_single_text(
                processed_pr_body,
                pull_request.bodyHtml,
